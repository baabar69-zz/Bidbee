<?php


class Login
{
    /**
     * @var object $db_connection The database connection
     */
    private $db_connection = null;
    /**
     * @var array $errors Collection of error messages
     */
    public $errors = array();
    /**
     * @var array $messages Collection of success / neutral messages
     */
    public $messages = array();

    private $userName = null;
    private $userId = null;
    private $userType = null;
    private $userStatus = null;

    public function __construct()
    {
        if (isset($_POST["AdminLoginForm"]))
        {
            $this->loginAdmin();
        }
        if (isset($_POST["UserLoginForm"]))
        {
            $this->loginUser();
        }


    }



    private function loginUser()
    {
        $email=$_POST["email"];
        $pswd=$_POST["pswd"];
        $this->db_connection = new mysqli(DB_HOST, DB_USER, DB_PASS, DB_NAME);

        $sql="Select * from users where email='$email' and password='$pswd' and type=0;";
        $query_verify = $this->db_connection->query($sql);
                   // $result_query = $query_verify->fetch_assoc();
                    while ($result_query = $query_verify->fetch_object()){
                        $name=$result_query->first_name;
                        $Id=$result_query->id;
                    }
                    if ($query_verify->num_rows == 1)
                    {
                        
                   $_SESSION['name'] = $name;
                   $_SESSION['userId'] = $Id;
                   header('Location: ../index.php');
                    }
                    else{

                        echo "Incorrect Email or Password. Try Again";
                    }







    }






    private function loginAdmin()
    {
        $email=$_POST["email"];
        $pswd=$_POST["pswd"];
        $this->db_connection = new mysqli(DB_HOST, DB_USER, DB_PASS, DB_NAME);

        $sql="Select * from users where email='$email' and password='$pswd' and type=1;";
        $query_verify = $this->db_connection->query($sql);
                   // $result_query = $query_verify->fetch_assoc();
                    while ($result_query = $query_verify->fetch_object()){
                        $name=$result_query->first_name;
                        $Id=$result_query->id;
                    }
                    if ($query_verify->num_rows == 1)
                    {
                        
                   $_SESSION['name'] = $name;
                   $_SESSION['userId'] = $Id;
                   header('Location: index.php');
                    }
                    else{

                        $this->errors[]= "Incorrect Email or Password. Try Again";
                    }
    //     if (empty($_POST['login_eMail'])){
    //         $this->errors[] = "Email Required";
    //     }elseif (empty($_POST['login_pwd'])){
    //         $this->errors[] = "Password Required";
    //     }
    //     else {
    //         //Create a database connection
    //       


    //         //if no connection errors (= working database connection)
    //         if (!$this->db_connection->connect_errno){

    //             $loginEmail= $_POST['login_eMail'];
    //             $loginPwd = $_POST['login_pwd'];

    //             //check if email already exists
    //           $sql = "SELECT * FROM users WHERE user_email = '$loginEmail' AND user_pwd = '$loginPwd';";
    //  
    //             if ($query_verify->num_rows == 1 && $this->userStatus == 0 ){

    //                $_SESSION['user'] = $this->userName;
    //                $_SESSION['userId'] = $this->userId;
    //                $_SESSION['userType'] =$this->userType;


    //                header("Location: index.php" );

    //             }elseif($this->userStatus == 2) {
    //                 $this->errors[] = "User is blocked.";

    //             }
    //             elseif($this->userStatus == 1)
    //             {
    //                 $this->errors[] = "User Status Pending.";


    //             }
    //         }else {
    //             $this->errors[] = "Sorry, no database connection.";
    //         }

    //     }








    }


}







?>